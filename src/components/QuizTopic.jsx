...
useEffect(()=>{
setIndex(0); setShowCard(true); setSelected(null); setRevealed(false); setScore(0); setFinished(false);
},[topicId]);
...
setFinished(true); setShowCard(false);
}else{
setIndex(ind=>ind+1);
...
<button className="btn" onClick={onBack}>← Topics</button>
<h2>{topic.title}</h2>
<div className="score-mini">Score: {score}/{topic.questions.length}</div>
</div>
